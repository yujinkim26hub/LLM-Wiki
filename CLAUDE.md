...
2. **Wiki** (everything in `10-Summaries/`, `30-Concepts/`, `40-Topics/`, `50-Notes/`) — your output. You create, update, link, and prune these freely.
...
40-Topics/          broad themes that gather concepts and sources
...
```yaml
---
type: concept | topic | summary | note
title: Display Title
...
**Links.** Use Obsidian wiki-link syntax: `[[concept-name]]` or `[[30-Concepts/concept-name|Display Text]]`. Prefer bare `[[concept-name]]` — Obsidian resolves it.
...
5. **Update `index.md`.** Add the new summary and any new concept/topic/note pages to the appropriate sections, with their one-line descriptions.
...
1. **Search the wiki first.** Use Grep over `10-Summaries/`, `30-Concepts/`, `40-Topics/`, `50-Notes/`. Read pages that look relevant.
...
4. **Missing cross-references.** Concepts mentioned in a summary but not linked to their concept page.
...
**`index.md`** is the catalog. Organized by category (Summaries, Concepts, Topics, Notes). Each entry is one line: `- [[page-slug]] — one-line description`. You update this on every ingest and every lint pass. It exists so you can navigate the wiki by reading one file, without having to grep blindly.
...
- **Prefer updating to creating.** Before creating a new concept page, grep to see if one exists under a different slug or alias. Add to it instead of creating a parallel page.
